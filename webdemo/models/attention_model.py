...
class AttentionModel(nn.Module):
    def __init__(self, num_classes=2000, question_vocab_size=13995, embedding_size=256, hidden_size=512):
        ...
        self.context_to_embedding_size  =  nn.Linear(hidden_size,embedding_size)
    ...
    def forward(self, question, image, hidden=None):
        ...
        for i in range(max_len):
            ...
            add_word_img = curr_word + self.context_to_embedding_size(context_sum.unsqueeze(dim=0))
            out, hidden = self.rnn(add_word_img, hidden)
        # print(out.shape)
        x = out.squeeze(dim=0)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
