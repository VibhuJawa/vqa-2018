...
def default_collate(batch):
    ...
    if torch.is_tensor(batch[0]):
        ...
        if 'word_count' and 'question' in batch[0]:
            max_len = max(map(lambda x: x['word_count'], batch))
            temp_dict['question'] = default_collate([d['question'][-max_len:] for d in batch])

        for key in batch[0]:
            if key == "question" or key == "word_count":
                continue
